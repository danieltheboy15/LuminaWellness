import { motion } from "motion/react";
import { ShoppingBag, HeartHandshake } from "lucide-react";
import { BRAND_CONFIG } from "../config";

export default function FinalCTA() {
  return (
    <section
      id="final-cta-section"
      className="px-4 sm:px-6 lg:px-8 py-24 bg-gradient-to-b from-[#150b21] via-[#100619] to-[#0a0212] border-t border-purple-950/40 relative overflow-hidden text-center text-white animate-fade-in"
    >
      {/* Background glow highlighting commitment block */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 sm:w-[500px] h-80 sm:h-[500px] bg-[#dfabeb]/10 rounded-full blur-[100px] sm:blur-[140px] -z-10 animate-pulse-slow" />

      <div className="max-w-4xl mx-auto py-16 px-6 sm:px-12 bg-zinc-950/30 backdrop-blur-md border border-purple-900/30 rounded-3xl relative overflow-hidden shadow-3xl">
        
        {/* Aesthetic background line */}
        <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-[#dfabeb]/20 to-transparent" />
        
        <div className="inline-flex p-3 bg-zinc-900 text-[#dfabeb] rounded-full mb-6 ring-1 ring-zinc-800">
          <HeartHandshake className="w-6 h-6" />
        </div>

        <h2 id="final-cta-heading" className="text-3xl sm:text-5xl font-display font-black text-white tracking-tight">
          Ready to start?
        </h2>

        <p className="mt-4 text-sm sm:text-base text-zinc-400 max-w-xl mx-auto font-light leading-relaxed">
          Take the first step today. Browse our treatments or message us. No pressure, just answers.
        </p>

        {/* Buttons / CTAs */}
        <div className="mt-10 flex flex-col sm:flex-row justify-center gap-4 max-w-sm sm:max-w-md mx-auto">
          <a
            id="final-cta-shop-btn"
            href={BRAND_CONFIG.catlogStoreUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2.5 py-4 px-8 rounded-full text-xs font-bold text-black bg-white hover:bg-[#dfabeb] active:scale-95 transition-all text-center font-display shadow-md cursor-pointer duration-205"
          >
            <ShoppingBag className="w-4 h-4 text-black" />
            <span>Shop Now</span>
          </a>

          <a
            id="final-cta-whatsapp-btn"
            href={BRAND_CONFIG.whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2.5 py-4 px-8 rounded-full text-xs font-semibold text-emerald-350 bg-emerald-950/40 hover:bg-emerald-950 border border-emerald-900/60 active:scale-95 transition-all text-center cursor-pointer duration-205"
          >
            <svg
              className="w-4 h-4 shrink-0 fill-current text-[#25D366]"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.746.953 3.71 1.454 5.709 1.455h.008c6.56 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            <span>Message us on WhatsApp</span>
          </a>
        </div>

      </div>
    </section>
  );
}
